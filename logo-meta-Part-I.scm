...
(define (collect-n-args n line-obj env paren-flag) ;; added parameter in 3
  (cond ((pair? n) (cons env (collect-n-args (car n) line-obj env paren-flag))) ;; added in 3
	((= n 0) '())	
	((and (< n 0) (not (ask line-obj 'empty?)))
	 (if paren-flag ;; added in 3
	     (let ((token (ask line-obj 'next)))
	     ...
	     (collect-n-args (- n) line-obj env paren-flag))) ;; added in 3
	(else      
      	 (let ((next (logo-eval line-obj env)))
           (cons next
	      	 (collect-n-args (- n 1) line-obj env paren-flag)) ))))
